...
Status: **em produção neste repositório**, migrado em 2026-09-20 de um monorepo maior (`whatsapp-mcp-hub`, que também continha o agente Hermes, notas de n8n e rotinas de Borderô misturadas) — porque WhatsApp é infraestrutura compartilhada, não uma feature de um site só. Governança elevada nesta migração: `whatsapp_enviar_texto` agora exige `confirmation='CONFIRMO'` além do flag de ambiente `WHATSAPP_MCP_ALLOW_WRITES`, que sozinho já foi encontrado ligado em produção sem confirmação por chamada. Deploy real validado pelo conector do claude.ai após a migração. Ver `00_READ_FIRST_WHATSAPP_MCP.md` seção 6 para o relato completo.
